...
In hindsight, CLIP feels less to me like a model milestone than like an early outline of a new paradigm. What it offered was not just a better vision system, but a different recipe: train for generality, learn a shared representation, and leave more room for downstream use to be specified at inference time.
...
Instead of mapping images to a finite label set, CLIP maps both images and text into a shared embedding space. Instead of predicting prescriptive categories, the model works against a more expressive space defined by language.
...
Prompt choice matters in practice for the same reason. Once the task is pushed to inference, phrasing starts to matter: whether you write "a photo of a dog," "a blurry photo of a dog," or "a cropped photo of a dog" can change the score distribution. OpenAI's paper leaned on prompt templates and prompt ensembling for exactly this reason.
...
<img
  src="/images/clip-paper/zs-vs-linear-clip_page1.png"
  alt="Zero-shot CLIP performance plotted against linear-probe CLIP performance."
...
Once images and text are aligned in a common space, many problems can be reduced to retrieval, ranking, and nearest-neighbor search. You can precompute image embeddings offline, index them in an approximate nearest-neighbor system, and turn "classification" or "search" into a similarity lookup. You can also compose CLIP with other components more cleanly: a ranker downstream, a generator upstream, or a lightweight task-specific model sitting on top of frozen embeddings. At that point, the older one-model-one-task picture starts to feel less natural.

You can see this clearly in systems like DALL·E 2, where CLIP embeddings are used as an intermediate representation between text and image generation. CLIP starts to look less like a task-specific model and more like a reusable building block.
